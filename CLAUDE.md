...
- **Phase 0 source pair**: SEC **Investor.gov glossary** (curated ~50-term Layer-1 subset) + Bank of England Knowledge Bank. Two deliberately different shapes — short glossary entries vs narrative explainers — to exercise the chunking pipeline against varied document structure. **Investor.gov replaced Investopedia at Step 003** (decided 2026-06-20): Investopedia sits behind a Cloudflare JS challenge that plain HTTP can't pass *and* its robots.txt name-blocks `ClaudeBot`/`anthropic-ai`, so it's neither cleanly fetchable nor clearly welcome to automated access. Investor.gov gives the same glossary shape, is **US-government public domain**, has clean robots (no AI-bot blocks), and is plainly fetchable — so the spec's automated fetch pipeline runs as designed. Investopedia is demoted to an **optional manual-save supplement** (the curated term list survives in `docs/investopedia-articles.md` for hand-saving specific entries later); Investor.gov terms are in `docs/investor-gov-terms.md`.
- **Usage & licensing**: personal use only, never published, always cited. RAG output must cite the original URL. Investor.gov content is **US-government public domain** (SEC) — the cleanest licensing of the set. BoE content is Crown copyright under the Open Government Licence (OGL) — verify exact terms during ingestion. Investopedia content (if hand-saved as an optional supplement) is © Investopedia (Dotdash Meredith), used under personal, non-commercial fair use.
...
- **Corpus reproducibility.** The corpus is reproducible from code + the URL list. `data/` is gitignored; anyone with the repo can reconstruct the same corpus by running ingestion. This is preserved by deterministic parser output (sorted lists in frontmatter, ISO 8601 UTC timestamps, a `parser_version` field) and is the answer to "why don't we just commit the parsed `.md` files?" — they're derivative content, and reproducibility lets us hold the fair-use line without losing portability.
...
- **Step 003** — Investor.gov glossary ingestion script (HTML fetch + clean).
